...
void doCommands(int day) {
    ...
    for (auto& kv : war) {
        ...
        if (majorityBases && techWorthNow && dist[myHQ][z] > dist[oppHQ][z]) continue;
    ...
    for (auto& kv : bld) {
        if (menace || armyBehind || eActiveField >= max(5, naturalHold + 2)) break;
...

    {
        ...
        bool defenseMode = earlyRush || menace || armyBehind || eActiveField >= max(5, naturalHold + 2);
