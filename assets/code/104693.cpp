...
map<int, int> razeCnt;
int prevRallyZone = -1;
int prevMultiDefZone = -1;
int prevBallTarget = -1;
int prevAttackTgt = -1;
map<int, int> razedEnemyRebuildUntil;
map<string, int> lastEnemyZone;


map<int, int> stagedSeen;
map<int, int> stagedLastCnt;
...
void doCommands(int day) {
    ...
    int eField = 0, eFieldOff = 0, eActiveField = 0, eArmyTotal = 0, spearHop = INT_MAX, spearZone = -1;
    int eStageMulti = 0;
    ...
    for (auto& kv : war) {
        ...
        bool onOppBld = bld.count(z) && bld[z].owner == oppC;
        if (onOppBld && onEnemyBld[z] < bldLabCap(bld[z])) { onEnemyBld[z]++; continue; }
        eField++;
        if (onOppBld) {
            if (!bld[z].hq) eStageMulti++;
            continue;
        }
        eFieldOff++;
        enemyFieldCnt[z]++;
    ...
    set<int> stagedThreatSet;
    {

        map<int, int> curExcess;
        for (auto& kv : bld) {
            int z = kv.first;
            if (kv.second.owner != oppC || kv.second.hq) continue;
            int ex = max(0, oppInZone(z) - bldLabCap(kv.second));
            if (ex > 0) curExcess[z] = ex;
        }


        for (auto it = stagedSeen.begin(); it != stagedSeen.end(); ) {
            if (!curExcess.count(it->first)) { stagedLastCnt.erase(it->first); it = stagedSeen.erase(it); }
            else ++it;
        }


        int stageLaunchLimit = (launchEta < 1000000000) ? launchEta * 2 + 4 : 20;

        int enemyHQLaunch = 1000000000;
        for (auto& b : bld)
            if (b.second.owner == meC && hopDist[oppHQ][b.first] < enemyHQLaunch)
                enemyHQLaunch = hopDist[oppHQ][b.first];
        for (auto& [z, ex] : curExcess) {

            if (!stagedSeen.count(z)) stagedSeen[z] = day;
            int prevEx = stagedLastCnt.count(z) ? stagedLastCnt[z] : 0;
            int duration = day - stagedSeen[z] + 1;

            int nearEx = ex;
            for (int nb1 : adj[z]) {
                nearEx += enemyFieldCnt[nb1];
                for (int nb2 : adj[nb1]) if (nb2 != z) nearEx += enemyFieldCnt[nb2] / 2;
            }

            int zLaunchEta = 1000000000;
            for (auto& b : bld)
                if (b.second.owner == meC && hopDist[z][b.first] < zLaunchEta)
                    zLaunchEta = hopDist[z][b.first];

            bool approachingFromZ = false;
            for (auto& kv : war) {
                if (kv.second.owner != oppC || kv.second.zone != z) continue;
                auto ip = lastEnemyZone.find(kv.first);
                if (ip == lastEnemyZone.end() || ip->second == z) continue;
                for (auto& b : bld) {
                    if (b.second.owner != meC) continue;
                    if (hopDist[z][b.first] < hopDist[ip->second][b.first]) { approachingFromZ = true; break; }
                }
                if (approachingFromZ) break;
            }

            bool persistOrGrowing = (duration >= 2) || (ex > prevEx);
            bool nearThreaten = (nearEx >= 3) || (ex >= 2 && duration >= 2);
            bool inRange = (zLaunchEta <= stageLaunchLimit);


            bool forwardStage = (enemyHQLaunch >= 1000000000) || (zLaunchEta <= enemyHQLaunch);
            if (persistOrGrowing && nearThreaten && inRange && (forwardStage || approachingFromZ)) {
                stagedThreatSet.insert(z);
            }
            stagedLastCnt[z] = ex;
        }

        for (auto& [z, _] : stagedLastCnt)
            if (!curExcess.count(z)) stagedLastCnt[z] = 0;
    }
    bool stagedThreat = !stagedThreatSet.empty();

    (void)eStageMulti;
    bool oppMassing = oppFreeArmy >= 4 && oppFreeArmy > mobileArmy;


    if (stagedThreat && !dominate) oppMassing = true;
    ...
    auto adjEnemyThreat = [&](int z) {
        int c = 0;
        for (int a : adj[z]) {
            auto ib = bld.find(a);
            if (ib != bld.end() && ib->second.owner == oppC) {
                int ex = oppInZone(a) - bldLabCap(ib->second);
                if (ex >= 2) c += ex;
                continue;
            }
            c += oppInZone(a);
    ...
    for (auto it = razedEnemyRebuildUntil.begin(); it != razedEnemyRebuildUntil.end();) {
        if (it->second < day || bld.count(it->first)) it = razedEnemyRebuildUntil.erase(it);
        else ++it;
    }
    auto canRebuildRazedEnemyBase = [&](int z) {
        if (!isBase[z] || bld.count(z)) return false;
        auto it = razedEnemyRebuildUntil.find(z);
        if (it == razedEnemyRebuildUntil.end() || it->second < day) return false;
        if (oppInZone(z) > 0 || adjEnemyThreat(z) >= 2) return false;
        int nearEnemy = 0;
        for (int a : adj[z]) {
    ...
    auto isRazedEnemyBase = [&](int z) {
        if (!isBase[z] || bld.count(z)) return false;
        auto it = razedEnemyRebuildUntil.find(z);
        return it != razedEnemyRebuildUntil.end() && it->second >= day;
        };
...

    {
        ...
        for (auto& kv : war) {
            ...
            double dxe = (double)(PX[pz] - PX[z]), dye = (double)(PY[pz] - PY[z]);
            double wEdge = ceil(sqrt(dxe * dxe + dye * dye));
            auto& ev = oppEvid[kv.first];
            for (auto& b : bld) {
                if (b.second.owner != meC) continue;
                int t = b.first;
                if (dist[pz][t] >= 1e17 || dist[z][t] >= 1e17) continue;
                if (fabs(wEdge + dist[z][t] - dist[pz][t]) < 1e-6) ev[t] += 1.0;
        ...
        for (auto& kv : war) {
            ...
            for (auto& b : bld) {
                if (b.second.owner != meC) continue;
                if (dist[z][b.first] < 1e17 && dist[pz][b.first] < 1e17 &&
                    dist[z][b.first] < dist[pz][b.first] - 1e-6) { zsz[z]++; break; }
            }
        }
        ...
        for (auto& cz : czones) {
            ...
            if (prevBallTarget >= 0 && prevBallTarget != bt && agg.count(prevBallTarget) &&
                bld.count(prevBallTarget) && bld[prevBallTarget].owner == meC &&
                agg[prevBallTarget] >= bc * 0.7 && agg[prevBallTarget] >= 2.0) {
                bt = prevBallTarget; bc = agg[prevBallTarget];
            }
            ...
            if (c.conf >= 2.0 && c.target >= 0)
                for (int z : cz.second) waveTarget[z] = c.target;
            clus.push_back(c);
        }
        sort(clus.begin(), clus.end(), [](const EClus& a, const EClus& b) { return a.size > b.size; });
        prevBallTarget = clus.empty() ? -1 : clus[0].target;
    ...
    if (earlyRush || menace || eFieldOff > 0)
        combatMoveReserve = 10LL * min(idleMy, max(3, eFieldOff + 2));


    if (stagedThreat && !menace && !earlyRush && eFieldOff == 0)
        combatMoveReserve = max(combatMoveReserve, 10LL * min(idleMy, max(3, (int)stagedThreatSet.size() * 2 + 2)));
    ...
    for (auto& kv : war) {
        ...
        if (oppInZone(z) > 0) continue;
        bool safeRazedBuild = razedEnemySpot && (canRebuildRazedEnemyBase(z) || dominate || myInZone(z) >= adjEnemyThreat(z) + 2);
        ...
        razedEnemyRebuildUntil.erase(z);
    }
    ...
    if (oppInZone(myHQ) == 0 && myInZone(myHQ) >= 1 && !usedUpg.count(myHQ)) {
        if (hl < 5 && !menace && !oppMassing) {
            ...
            if ((econOK || hl < eHQlv) && avail >= need + buffer) {
                cout << "UPGRADE " << myHQ << "\n";
    ...
    if (stagedThreat && !menace && !ballConfMulti) {
        int bestD = 1000000000;
        for (int sz : stagedThreatSet) {
            for (auto& kv : bld) {
                if (kv.second.owner != meC) continue;
                int d = hopDist[kv.first][sz];
                if (d < bestD) { bestD = d; rallyZone = kv.first; }
            }
        }
    ...
    if (menace && ballConfMulti && bld.count(clus[0].target) && bld[clus[0].target].owner == meC) {
        rallyZone = clus[0].target;
    }
    else if (menace && mainThreatZone >= 0) {
    ...
    if (!menace && !dominate && ballConfMulti &&
        bld.count(clus[0].target) && bld[clus[0].target].owner == meC)
        rallyZone = clus[0].target;
    ...
    for (auto& kv : bld) {
        ...
        if (isHQ) {
            ...
            for (auto& w : war) {
                ...
                auto wt = waveTarget.find(wz);
                if (wt != waveTarget.end() && wt->second != z) continue;
    ...
    if (worstOver > 0 && prevMultiDefZone >= 0 && defZone != prevMultiDefZone &&
        bld.count(prevMultiDefZone) && bld[prevMultiDefZone].owner == meC && !bld[prevMultiDefZone].hq &&
        dist[myHQ][prevMultiDefZone] <= dist[oppHQ][prevMultiDefZone] && !evacuate.count(prevMultiDefZone)) {
        for (auto& f : fronts) if (f[2] == prevMultiDefZone) {
            defZone = prevMultiDefZone; needStaff = f[3]; break;
        }
    }
    prevMultiDefZone = (worstOver > 0 && bld.count(defZone) && !bld[defZone].hq) ? defZone : -1;
    ...
    if (counterRace) { defZone = rallyZone; needStaff = 0; worstOver = 0; hqOver = 0; hqNeed = 0; fronts.clear(); }
    ...
    for (int z : expansionOrder) {
        ...
        if (dist[myHQ][z] > dist[oppHQ][z]) continue;
        if (dist[myHQ][z] > dist[oppHQ][z] * expandReach) continue;
        if (oppInZone(z) > 0 || adjEnemyThreat(z) > 0) continue;
    ...
    for (auto& kv : war) {
        ...
        if (z == myHQ) {
            ...
            if (canRebuildRazedEnemyBase(z) && kept[z] < 1) { kept[z]++; continue; }
            ...
            if (oppInZone(z) == 0 && adjEnemyThreat(z) == 0 &&
    ...
    for (auto& kv : bld) {
        ...
        if (adjEnemyThreat(z) > 0) continue;
    ...
    for (auto& pr : razedEnemyRebuildUntil) {
        int z = pr.first;
        if (fi >= freeW.size() || avail < 10) break;
        if (!canRebuildRazedEnemyBase(z) || staffOf(z) > 0) continue;
        if (dist[oppHQ][z] < dist[myHQ][oppHQ] * 0.20) continue;
...
void parseResult() {
    ...
    for (auto& [t, d] : ups) {
        ...
        if (t == meC) {
            razedEnemyRebuildUntil.erase(d);
    ...
    for (auto it = bld.begin(); it != bld.end();) {
        if (it->second.hp <= 0) {
            ...
            else if (it->second.owner == oppC && !it->second.hq) razedEnemyRebuildUntil[it->first] = T + 12;
            it = bld.erase(it);
        }
        else ++it;
    }
