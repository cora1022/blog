...
static void phaseBuild(long long& budget, set<int>& done, int& fortCnt, const Danger& D, int& hqLvl, int day) {
    int goal = fortTarget();

    int hqCap = (day > 140) ? min(CFG.hqTop, 2) : CFG.hqTop;
    ...
    if (!D.besieged) {
        long long hqReserve = (hqLvl < hqCap) ? R.hqUpgrade[hqLvl + 1] : 0;
        for (auto& b : W.base) {
...
static void phaseMint(long long& budget, int fortCnt, const Danger& D, int hqLvl) {
    ...
    long long earmark = (fortCnt < fortTarget())
                         ? (long long)R.fcBuild[1] * (fortTarget() - fortCnt)
                         : 0;
    ...
    long long moveReserve = D.besieged ? 0 : 20;
    long long free = budget - earmark - moveReserve;
...
static void phaseMove(long long& budget, const Danger& D, int hqLvl, int day) {
    ...
    bool haveAnyFort = false;
    for (auto& b : W.base)
        if (b.second.flag == W.me && !b.second.capital) { haveAnyFort = true; break; }
    bool canRebuild = false;
    if (!haveAnyFort) {
        for (int z = 0; z < W.cells; z++) {
            if (!W.fortable[z] || z == cap) continue;
            if (W.base.count(z)) continue;
            if (!W.inMyReach(z)) continue;
            canRebuild = true; break;
        }
    }
    bool allIn = W.fortLost && !haveAnyFort &&
                 (day >= 160 || !canRebuild);
    ...
    auto pickTarget = [&]() -> int {
        ...
        for (auto& b : W.base) {
            ...
            int z = b.first;
            if (W.reachFoe[z] >= 1e17) continue;
            double d = W.reachFoe[z];

            if (d < bestD - 1e-9 ||
                (fabs(d - bestD) <= 1e-9 && best >= 0 && W.reachMine[z] < W.reachMine[best])) {
                bestD = d; best = z;
            }
        }
    ...
    for (auto& b : W.base) {
        if (b.second.flag != W.foe || b.second.capital) continue;
        int z = b.first;
        if (W.hereMine(z) > 0 && W.reachFoe[z] < engD) { engD = W.reachFoe[z]; engaged = z; }
    }
    if (engaged >= 0) {
    ...
    if (D.hqEmergency) {
        ...
        int dz = D.hitBase;
        int need = max(2, W.hereFoe(dz) + 1);
        int have = W.boundFor(dz);
        while (k < idle.size() && have < need) {
            int sp = W.army[idle[k]].spot;
            if (sp == dz) { k++; continue; }

            if (sp == cap || sp == f1 || sp == f2) { order(idle[k++], dz, budget); have++; }
            else k++;
        }
    }


    int rally     = allIn ? cap : (ownFort(f2) ? f2 : (ownFort(f1) ? f1 : cap));
    int atRally   = W.hereMine(rally);
    int launchAt  = allIn ? 4 : 6;
    int rushTgt   = allIn ? capFoe : atkTarget;
    ...
    if (allIn) {
        if (atRally >= launchAt) {
            ...
            int keepHome = D.hqEmergency ? hqKeep : 0;
            int sendN = (int)here.size() - keepHome;
            for (size_t i = 0; i < here.size() && sendN > 0; ++i) {
                order(here[i], rushTgt, budget); sendN--;
            }
        }
    } else {
        const int LAUNCH = 5;
...
static void playTurn(int day) {
    ...
    {
        auto own = [&](int z){ return z>=0 && W.base.count(z) && W.base[z].flag==W.me && !W.base[z].capital; };
        bool haveFort = own(W.fort1) || own(W.fort2);
        if (haveFort) { W.fortEverBuilt = true; W.fortLost = false; }
        else if (W.fortEverBuilt) W.fortLost = true;
    }


    phaseBuild(budget, done, fortCnt, D, hqLvl, day);
